...
The lower left plot is a scale-location plot. The regression assumes homoscedasticity, that the variance in the residuals doesn’t change as a function of x. If that assumption is correct, then the red line should be relatively flat, the red line is flat here. This shows that the errors are homoscedasticity.
...
@
Trial might  be  treated  as  a  categorical  variable  with  four  levels,  or  as  a
