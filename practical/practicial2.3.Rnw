...
In the ”zeroth” quarter the expected number of aids cases is β1. This must
be a fairly small number, around 1 corresponding to the index case, since this is the beginning of the
epidemic. As we move from quarter 13 to quarter 14 the number of aids cases from order (1 + 13β2)
to (1 + 14β2) so the increase is about (2 + 27β)β2. If β2 is as large as 10 this is in the thousands which
seems very large considering this is year four. I am a bit uncertain though so I will take βi ∼ 2.5×t(1),
that is, a t-distribution scaled by 2.5 (a 90% chance βi < 8 and 95if the data support it). I might
reasonably also condition β2 > 0 but I have not done this.
