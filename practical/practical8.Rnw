...
\end{figure}
We remove the sex from figure 1, and we get figure 2 now. The black dots represent Non-Standard, and red dots represent Standard, we don't see the moratluty is different for Standard variable in figure 2.
...
\end{figure}
we draw the boxplot of moratluty with sex and standard. From this boxplot(figure 3), we also can see that standard dosen't affect the moraluty a lot. For sex variable, we see females has a large variance compare to males. So in our initial model, we will consider dose and sex as exploanatory variables.
...
To investigate how mortality depends on dose, we fit the mortality and dose into logistic regression. Based on what we find from Data exploration part, we decide our initial model to be
\begin{equation}
(numdead, numalive) \sim I(dose^2) + dose
\end{equation}

The residual deviance of model above is 20.44 with degree of freedom 21. We use deviance test to compare this model to satruated model, the p-value we get is 0.49, which is less than 0.05, so we aceept the intial model at 0.05 significant level. We take this model as suitable model for risk of mortality based on dose.

Now we consider the other variables' relationship with mortality. As we discovered in Data exploration part, we add sex into our initial model and do the StepAIC.
...
The StepAIC gives us:
\begin{equation}
(numdead, numalive) \sim I(dose^2) + dose + sex + I(dose^2):sex + dose:sex + I(dose^2):dose
\end{equation}
The model above gives us the smallest AIC. We can see this model only includes dose and sex, it matches what we find in exploration part. But instead of using AIC stepwise, we also need to look at the ANOVA of this model based on Chi-squared test. 
...
\begin{tabular}{lrrrrr}
 ...
 & Df & Deviance & Resid. Df & Resid. Dev & Pr($>$Chi) \\ 
  \hline
NULL &  &  & 23 & 84.21 &  \\ 
  I(dose\verb|^|2) & 1 & 7.33 & 22 & 76.88 & 0.00 \\ 
  dose & 1 & 56.44 & 21 & 20.44 & 0.00 \\ 
  sex & 1 & 0.66 & 20 & 19.78 & 0.42 \\ 
  I(dose\verb|^|2):sex & 1 & 3.88 & 19 & 15.90 & 0.05 \\ 
  dose:sex & 1 & 9.07 & 18 & 6.83 & 0.00 \\ 
  I(dose\verb|^|2):dose & 1 & 2.33 & 17 & 4.50 & 0.13 \\ 
   \hline
   \caption{\textbf{ANOVA table of model3}}
\end{tabular}
\end{table}

From the ANOVA table of model3, we can see that I(dose\verb|^|2):dose is not significant at 0.05 level based on deviance test, so we consider to remove this term from our model. Even the sex term is also not significant, but interaction terms of sex(I(dose\verb|^|2):sex, dose:sex) are significant, so we keep sex in our model. Our finaly model is:
\begin{equation}
...
\begin{tabular}{rrrrr}
 ...
 & Estimate & Std. Error \\ 
  \hline
(Intercept) & 3.29 & 0.58 \\ 
  I(dose\verb|^|2) & 0.36 & 0.05\\ 
  dose & -2.69 & 0.39 \\ 
  sexM & -1.92 & 0.79 \\ 
  I(dose\verb|^|2):sexM & -0.24 & 0.07 \\ 
  dose:sexM & 1.58 & 0.53 \\ 
   \hline
...
Since we have interaction between sex and dose in our model, we have two equations for males and females separately:

\begin{equation}
Female : (numdead, numalive) = 3.29 + 0.36*I(dose^2) - 2.69*dose
\end{equation}
...
For Female rats, if rats don't receive any treatment, the log odd of mortality is 3.29. Since it is a qurdratic function, $b^2 - 4ac = 2.69^2-4*3.29*0.36 = 2.5$ is greater than zero, this equation has a minimum point. We need to calculte the first derivative and set it equal to zero to get the minimum point. 0.36*2*dose-2.69 = 0, from this equation, we get $dose = 3.73 \approx 4$. When dose equal to 4, we have the smallest log odd of moratality. It matches what we see in figure 1. 

For Male rats, we do the same analysis. If rats don't receive any treatment, the log odd of mortality is 3.29 - 1.92 = 1.37. Since it is a qurdratic function, $b^2 - 4ac = (-2.69+1.58)^2 - 4*(3.29-1.92)*(-2.69+1.58) = 7.31$ is greater than zero, (0.36-0.24)*2*dose-(-2.69+1.58) = 0, from this equation, we get $dose = 4.62 \approx 5$. When dose equal to 5, we have the smallest log odd of moratality. It also matches what we see in figure 1. 

Now we compare the Female and male model. 
...
\begin{tabular}{rrr}
 ...
 & 2.5 \% & 97.5 \% \\ 
  \hline
(Intercept) & 2.19 & 4.47 \\ 
  I(dose\verb|^|2) & 0.26 & 0.47 \\ 
  dose & -3.48 & -1.95 \\ 
  sexM & -3.48 & -0.39 \\ 
  I(dose\verb|^|2):sexM & -0.39 & -0.10 \\ 
  dose:sexM & 0.55 & 2.63 \\ 
   \hline
   \caption{\textbf{confidence interval of parameters}}
\end{tabular}
\end{table}
