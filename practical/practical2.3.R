...
par(mfrow = c(1,2))
plot(apply(mu.sima,2,mean), xlab = "Each rectangle of fields", ylab = "simulated values",
     main = "Check the normal prior by simulation")
...
lpr<-function(beta) {
  sum(dnorm(beta[2:13],0,1,log = T), dnorm(beta[1],1,1,log = T))
}
...
par(mfrow=c(1,2))
plot(LP[1:400],type='l', xlab = "index of MCMC sample", ylab = "log posterior")
plot(LP[seq(400,K,10)],xlab = "index of MCMC sample", ylab = "log posterior"); 
...
par(mfrow=c(3,1))
plot(B[,3], type = "l", ylab = "estimated parameter value", xlab = "iteration", 
     main = "Beta 3 trace plot")

acf(as.mcmc(B[,1:3]),lag.max=3000)
