...
points(rats[rats$sex=='F'&rats$standard==0,"dose"],rats[rats$sex=='F'&rats$standard==0,"dead_rate"],pch=5,col='red')
legend("topright", col = c('blue', 'blue', 'red','red'), pch = c(1,0,2,5), legend = c("Male and Standard", "Male and Non Standard", 
...
# we see a quadratic relationship between does and dead-rate, so apply quadratic model
...
###########################################  4
#Comment on your model and interpret the parameters giving suitable measures of uncertainty
...
#anova(justice.glm<-glm(formula = cbind(numdead,numalive)~I(dose^2) * sex * standard * dose, 
#                       family = binomial(), data = rats),test='Chisq')
...
plot(model4$fitted.values,model4$residuals,xlab='Fitted Values', ylab='Working Residuals',
     pch=19,xlim=c(0,1),ylim=c(-1.5,1.5))
abline(h=0)
#ok
...
plot(ndM$dose,bw.respM*20,type='l',col='blue',ylim=c(0,20), xlab='Dose level', ylab='Proportion dead')
